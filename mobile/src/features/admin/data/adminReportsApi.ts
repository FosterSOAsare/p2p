...
import { adminListingKeys, type ListingStatus, type RemovalReason } from './adminListingsApi';

/**
 * Buyer reports on listings — the phone side of
 * `web/src/features/admin/data/adminReportsApi.ts`.
 *
 * Grouped by listing, not by report: a listing flagged eight times is one
 * decision, not eight, so the server does the grouping and the queue reviews a
 * listing at a time.
 */
