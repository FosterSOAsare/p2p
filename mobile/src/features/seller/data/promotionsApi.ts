 ...
 * to draw a live preview while the seller moves the boost stepper — the same
 * trick the escrow form uses for the fee — and the server re-prices every charge
 * from scratch, so a stale mirror can never become a wrong debit.
 */
...
/** The server validates rank against this same grid, so the stepper must honour it. */
...
/** Headline for the current boost rank — the studio's "what am I buying" line. */
export function getBoostTier(score: number) {
...
/** Launching debits the wallet, so the balance and the seller's own numbers move too. */
...
/**
 * Live price for the current stepper position. Enabled only once a plan and a
 * listing are known; the studio falls back to `promotionPrice` while it loads.
 *
 * The rank is part of the key, so every step is a fresh key. Holding the
 * previous answer while the new one loads keeps `data` defined throughout —
 * otherwise the studio watches `current` blink to undefined and back on every
 * step, which is enough to make it re-adopt the saved rank and undo the edit.
 */
